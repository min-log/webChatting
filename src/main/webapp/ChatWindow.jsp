...
<script type="text/javascript">
	...
	// 작업 순서 
	//1. 웹 소켓 생성
	let webSocket = new WebSocket("<%= application.getInitParameter("CHAT-ADDR")%>/ChatingServer");
	let chatWindow,chatMessage,chatId;
	
	
	//2. 채팅창이 열리면 ( 대화창, 메시지 입력창, 대화명 표시 dom 객체 저장 )
	...
	//5. 앤터 키 입력 처리
		// 앤터 키 누를 시 sendMessage() 호출 --> 메시지 전송 
	function enterKey(){
	...
	//서버와 연결되는 기능 ----------------------
	//6. 웹 소켓 서버에 연결 시 실행되는 메서드 연결
	webSocket.onopen = function(event){
		chatWindow.innerHTML += "<p>웹 소켓 서버에 연결되었습니다.</p>";
	...
	webSocket.onclose = function(event){
		chatWindow.innerHTML += "<p>웹 소켓 서버가 종료되었습니다.</p>";
	}
	...
	webSocket.onerror = function(event){
		alert(event.data);
		chatWindow.innerHTML += "<p>채팅중 오류가 발생 하였습니다.</p>";
	}
